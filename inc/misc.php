...
function custom_get_additional_fees_settings() {
    ...
    $additional_fees_settings = array(
        ...
        'priority_delivery' => array(
            'name' => __('Expressversand', 'nutrisslim-suite'),
            'type' => 'number',
        ...
        'package_insurance' => array(
            'name' => __('Versandversicherung', 'nutrisslim-suite'),
            'type' => 'number',
...
function add_custom_fees_to_cart() {
    ...
    $custom_fees = [
        ...
        'rf_zavarovanje_narocila' => [
            'label' => __('Versandversicherung', 'nutrisslim-suite'),
        ...
        'rf_priority_delivery' => [
            'label' => __('Expressversand', 'nutrisslim-suite'),
...
function custom_checkout_additional_options($checkout) {
    ...
    ?>
        ...
        <div id="delivery-option" class="rf-custom-option">
            <h3><?php echo __('Lieferung upgraden', 'nutrisslim-suite'); ?></h3>
                ...
                <div class="delivery-option rf-option-container" style="<?php echo $is_checked_ekoloska_bg; ?>">
                    <div class="rf-option-left">
                        ...
                        <label for="rf_ekoloska_embalaza">
                            <div class="rf-option-title"><?php echo __('Umweltfreundliche Verpackung', 'nutrisslim-suite'); ?>
                                (+<?php echo wc_price($eco_friendly_packaging); ?>)</div>
                            <div class="rf-option-description">
                                <?php echo __('Erhalten Sie Produkte in biologisch abbaubarer, umweltfreundlicher Verpackung und helfen Sie mit, die Natur zu schützen.', 'nutrisslim-suite'); ?>
                            </div>
                        </label>
                    </div>
                ...
                <div class="delivery-option rf-option-container" style="<?php echo $is_priority_delivery_bg; ?>">
                    <div class="rf-option-left">
                        ...
                        <label for="rf_priority_delivery">
                            <div class="rf-option-title"><?php echo __('Überspringen Sie die Warteschlange', 'nutrisslim-suite'); ?>
                                (+<?php echo wc_price($priority_delivery); ?>)</div>
                            <div class="rf-option-description">
                                <?php echo __('Wählen Sie die Prioritätsverpackung für den schnellsten Versand. Durch die Auswahl dieser Option wird Ihre Bestellung vor anderen bearbeitet.', 'nutrisslim-suite'); ?>
                            </div>
                        </label>
                    </div>
                ...
                <div class="delivery-option rf-option-container" style="<?php echo $is_checked_zavarovanje_bg; ?>">
                    <div class="rf-option-left">
                        ...
                        <label for="rf_zavarovanje_narocila">
                            <div class="rf-option-title"><?php echo __('Paketversicherung', 'nutrisslim-suite'); ?>
                                (+<?php echo wc_price($package_insurance); ?>)</div>
                            <div class="rf-option-description">
                                <?php echo __('Sichern Sie die Lieferung der Sendung ab und erhalten Sie eine garantierte und kostenlose Lösung im Falle von beschädigten oder fehlenden Produkten. Wir versenden das Paket ohne zusätzliche Kosten für Sie.', 'nutrisslim-suite'); ?>
                            </div>
                        </label>
                    </div>
...
<?php } ?>
    ...
    <div class="rf-custom-option select-shipping">
        <h3><?php echo __('Lieferung', 'woocommmerce'); ?></h3>
...
function custom_woocommerce_loop_add_to_cart_link($html, $product) {
    ...
    $custom_text = __( 'In den Warenkorb', 'woocommerce' );
...
function cart_upsell() {
    ...
    if (!empty($items)) {
        ...
        $output .= '<h3>' . __('Sonderangebot nur hier!', 'nutrisslim-suite') . '</h3>';
        $output .= '<p>' . __('Hinzufügen, um zu bestellen und zu sparen!.', 'nutrisslim-suite') . '</p>';
        ...
        foreach ($items as $item) {
            ...
            $output .= '<button class="cart-button_swiper add-to-cart-icon" data-offer="cart_upsell" data-product-id="' . $pid . '" data-quantity="1" data-price="' . $offer_price . '" ' . $disabled . '>' . __('In den Warenkorb', 'woocommerce') . '</button>';
...
function add_meta_to_order_fees($item, $fee_key, $fee, $order) {
    ...
    $fee_sifrant_map = array(
        ...
        'expressversand' => get_option('priority_delivery_fee_sifrant'),
        'eco-friendly-packaging' => get_option('custom_eco_friendly_packaging_sifrant'),
        'versandversicherung' => get_option('custom_package_insurance_sifrant'),
...
/**
 * Get review data; optionally force an array return.
 *
 * @param string $sku
 * @param bool   $asArray If true: always return an array (empty if missing). If false: return array|null.
 * @return array|null
 */
function get_review_data_with_cache(string $sku, bool $asArray = false): ?array
{
    if ( ! function_exists('ns_get_cumulative_review_data_local') ) {
        return $asArray ? [] : null;
    }

    $data = ns_get_cumulative_review_data_local($sku);
    ...
    // Not forcing array: only return if it’s already an array; otherwise null.
