...
function customize_checkout_fields($fields) {
    ...
    $fields['billing']['billing_email'] = array(
        'type'        => 'email',
        'label'       => __('E-Mail-Adresse', 'woocommerce'),
        'placeholder' => __('E-Mail-Adresse*', 'woocommerce'),
        'required'    => true,
    ...
    if ($default_country == 'GB') {
        $fields['billing']['billing_address_1'] = array(
            'type'        => 'text',
            'label'       => __('Straßenadresse', 'woocommerce'),
            'placeholder' => __('Straßenadresse*', 'woocommerce'),
            'required'    => true,
        ...
        $fields['billing']['billing_house_number'] = array(
            'type'      => 'text',
            'label'     => __('Hausnummer', 'nutrisslim-suite'),
            'placeholder' => __('Hausnummer*', 'nutrisslim-suite'),
        ...
        $fields['billing']['billing_postcode'] = array(
            ...
            'placeholder' => __('Postleitzahl *', 'woocommerce'),
            'required'    => true,
            'class'       => array('form-row-first'),
            'clear'       => true,
            'priority' => 60,
        ...
        $fields['billing']['billing_city'] = array(
            'label'       => __('Rechnungsstadt', 'woocommerce'),
            'placeholder' => __('Rechnungsstadt*', 'woocommerce'),
            'required'    => true,
            'class'       => array('form-row-last'),
            'priority' => 70,
        );
    ...
    $fields['billing']['billing_phone'] = array(
        ...
        'placeholder' => __('Telefon*', 'woocommerce'),
        'required'    => true,
        'class'       => array('form-row-wide'),
        'priority'    => 80,
    );
    ...
    $fields['order']['order_comments'] = array(
        ...
        'placeholder' => __('Anmerkungen zu Ihrer Bestellung, z. B. besondere Hinweise zur Lieferung.', 'woocommerce'),
        'priority'    => 100,
    );
...
add_filter('woocommerce_checkout_fields', function($fields) {
    if (isset($fields['billing']['billing_city'])) {
        unset($fields['billing']['billing_city']['validate']); // Remove any validation
    }
    return $fields;
}, 9999);
            ...
            <div class="rf-option-container"' . $is_colored . '>
                <div class="rf-option-left">
                    ...
                    <label for="rf_' . esc_attr($free_shipping_method->id) . '">
                        <div class="rf-option-title">' . esc_html($free_shipping_method->label) . $type . ' (' . esc_attr(__('Gratis', 'woocommerce')) . ')</div>
                        <div class="rf-option-description"></div>
                    </label>
                </div>
            </div>';
...
function add_h3_order_summary_title() {
    ...
    echo '<h3 id="payment_method_heading_inner">' . esc_html__('Zahlungsmethoden', 'woocommerce') . '</h3>';
...
function additional_elements() {
    echo '<h3 id="order_review_heading_inner">' . esc_html__('Ihre Bestellung', 'woocommerce') . '<span class="toggle">' . esc_html__('Anzeigen / Ausblenden', 'nutrisslim-suite') . '</span></h3>';
    echo '<p id="final-message">' . esc_html__('Dies ist eine Bestellung mit Zahlungsverpflichtung. Durch das Absenden stimmen Sie dem zu. Wir verpflichten uns, Ihre Privatsphäre zu schützen und zu respektieren. Daher verwenden wir personenbezogene Daten ausschließlich zur Verwaltung Ihrer Bestellung oder Ihres Kontos sowie zur Bereitstellung von Informationen über die von Ihnen bestellten Produkte.', 'nutrisslim-suite') . '</p>';
}
...
function get_checkout_offer_ajax() {
        ...
        foreach ($items as $item) {
            ...
            echo '<img decoding="async" src="/wp-content/uploads/2024/04/Home-Ikone-NOVO_cart.svg" alt="Dodaj do koszyka" style="vertical-align: middle; margin-right: 8px;">' . __('In den Warenkorb', 'woocommerce') . '</a>';
            echo '</div>'; // col-12
