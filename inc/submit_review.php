    ...
    function filter_submit_review_page_content( string $content ): string {
        ...
        <div class="mass-reviews container container-px">
            <div class="review-intro">
                <h1 class="bold"><?php echo __('Ihre Meinung ist uns wichtig', 'nutrisslim-suite'); ?></h1>
                <p><?php echo __('Bei Nature\'s Finest bemühen wir uns stets, die besten Produkte und Dienstleistungen für unsere geschätzten Kundinnen und Kunden bereitzustellen. Möchten Sie Ihren letzten Einkauf bewerten?', 'nutrisslim-suite'); ?></p>
                <p><?php echo __('Als Dank für Ihre Zeit legen wir Ihrer Bestellung ein besonderes Geschenk bei!', 'nutrisslim-suite'); ?></p>
                <h4><?php echo __('Ihre Bestellung', 'woocommerce'); ?></h4>
                ...
                <h4><?php echo __('Bewerten und rezensieren Sie Ihren Einkauf', 'nutrisslim-suite'); ?></h4>
            </div>
            <div class="form">
                <div class="nf-review_form">
                    <div class="review_fields">
                        ...
                        <textarea name="review_review" id="review_text" cols="30" rows="5" placeholder="<?php echo __('Teilen Sie uns Ihre Meinung mit','nutrisslim-suite'); ?>"></textarea>
                        <button id='send_reviews'><?php echo __("Senden","woocommerce"); ?></button>
                    </div>
                </div>
            </div>
        </div>
...
if ( ! function_exists('handle_submit_reviews') ) {
    function handle_submit_reviews(): void {
        ...
        if ( $inserted > 0 ) {
            wp_send_json_success( 'Die Bewertungen wurden erfolgreich gesendet..' );
        }
        wp_send_json_error( 'Es konnten keine Bewertungen hinzugefügt werden (möglicherweise doppelt oder Fehler bei der Eingabe).' );
    }
