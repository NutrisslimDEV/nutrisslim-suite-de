...
<?php
    ...
    if ($product_type == 'nutrisslim') {
        // $singleLabel = ' paket';
        $singleLabel = __('pakete', 'nutrisslim-suite');
        $dvaLabel = 'pakete';
        // $triLabel = ' paketi';
        $triLabel = __('pakete', 'nutrisslim-suite');
    } else {
        // $singleLabel = ' kom';
        $singleLabel = __('kunst', 'nutrisslim-suite');
        $dvaLabel = 'kom'; 
        $triLabel = 'kom';                    
        // $triLabel = ' kom';                    
        $triLabel = __('kunst', 'nutrisslim-suite');
    }
...
<div class="customAddToCart">
    <div class="topInfo">
        ...
        <span class="subscriptiolLabel" style="display:none;">
            <?php printf(__('für <span class="numdays">%s</span> tage', 'nutrisslim-suite'), $subscription_periods[0]); ?></span>
    </div>
    <p><?php echo __('Wählen Sie die gewünschte Menge aus:', 'nutrisslim-suite'); ?></p>
        ...
        <form>
            <div class="qSelect grid-3">
                ...
                <label class="col top">
                    ...
                    <div class="paket">
                        ...
                        <div class="choice">
                            <?php echo __('Die beste Wahl', 'nutrisslim-suite'); ?>
                        </div>
                    </div>
                </label>
                <?php        
            ...
            <div class="tabsHolder">
                <div class="tabsItems grid-2">
                    <div class="col tab subscr">
                        <?php echo __('Abonnieren und sparen', 'nutrisslim-suite'); ?><br />
                        <span class="warning">-<?php echo $discount_one_subscription; ?>%
                            <?php echo __('RABATT', 'nutrisslim-suite'); ?><br /></span>
                    </div>
                    <div class="col tab active onetime"><?php echo __('Einmalkauf', 'nutrisslim-suite'); ?>
                    </div>
                </div>
                <div class="contWrap">
                    <div class="onetimeContent">
                        <p>
                            <strong>
                                <?php 
                                    printf( 
                                        __( 'Wechseln Sie zum Abonnement und sparen Sie %s', 'nutrisslim-suite' ), 
                            ...
                            <?php 
                                printf( 
                                    __( 'Sie können dasselbe für nur %s haben. Eine bequeme Möglichkeit, die besten Preise zu erhalten. Das Abonnement kann jederzeit geändert oder gekündigt werden!', 'nutrisslim-suite' ), 
                    ...
                    <div style="display:none;" class="subscrContent">
                        <p><?php echo __('Wählen Sie den Abonnementzeitraum:', 'nutrisslim-suite'); ?></p>
                        <select name="subscription" class="subscription">
                            <?php
                                    foreach ($subscription_periods as $subscription_period) {
                                        echo '<option value="' . $subscription_period . '">' . __('Wähle den Abonnementzeitraum:', 'nutrisslim-suite') . ' ' . $subscription_period . ' ' . __('tage', 'nutrisslim-suite') . '</option>';
                                    }
                                ?>
                        </select>
                        <div class="grid info">
                            <div class="col-6 cancel">
                                <?php echo __('Das Abonnement kann jederzeit gekündigt werden.', 'nutrisslim-suite'); ?>
                            </div>
                            <div class="col-6 lowest">
                                <?php echo __('Verifizierter niedrigster Preis', 'nutrisslim-suite'); ?>
                            </div>
                            <div class="col-12 learn"><a class="moreabout"
                                    href="#"><?php echo __('Erfahren Sie mehr über das Abonnement', 'nutrisslim-suite') ?></a>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <?php
                    if ( !$product->is_in_stock() ) {
                        ...
                        echo '<p style="text-align:center"><span class="out-of-stock">' . __( 'Nicht auf Lager', 'woocommerce' ) . '</span></p>';
                    } else {
                ?>
            <button class="elementor-button add-to-cart-icon" data-product-id="<?php echo $product->get_id(); ?>"
                data-quantity="1" type="submit"><?php echo __('In den Warenkorb', 'woocommerce'); ?> | <span
                    class="buttonPrice"><?php echo wc_price($sale_price); ?></span> <span class="buttonTime"
                    style="display:none;"><?php printf(__('Alle <span class="numdays">%s</span> tage', 'nutrisslim-suite'), $subscription_periods[0]) ?></span></button>
            <?php
            ...
            <?php
                    if ( $product->is_in_stock() ) {
                        // If the product is out of stock, display "Out of Stock" text
                        echo '<p class="onstock">' . __( 'Auf Lager, versandbereit', 'nutrisslim-suite' ) . '</p>';
                    }                
                ?>
    ...
    <div class="featureWrapper">
        <div class="feature return"><img src="/wp-content/plugins/nutrisslim-suiteV2/assets/cart_vracilo_ico.svg" />
            <?php printf(__('%s-Tage Geld-zurück-Garantie', 'nutrisslim-suite'), 60); ?></div>
        <div class="feature free"><img src="/wp-content/plugins/nutrisslim-suiteV2/assets/cart_free_delivery_ico.svg" />
            <span><?php printf(__('Kostenloser Versand bei Bestellungen über %s', 'nutrisslim-suite'), wc_price(get_free_shipping_threshold())); ?></span>
        </div>
        <div class="feature delivery"><img src="/wp-content/plugins/nutrisslim-suiteV2/assets/cart_dostava_ico.svg" />
            <?php printf(__('Lieferung innerhalb von 3-4 Werktagen', 'nutrisslim-suite'), '1-2'); ?></div>
    </div>
    ...
    <p class="phone">
        <?php printf(__('Bestellungen und Anfragen können unter der Nummer gestellt werden <strong>%s</strong>', 'nutrisslim-suite'), get_option('woocommerce_support_phone')); ?>
    </p>
</div>
<?php
    }
