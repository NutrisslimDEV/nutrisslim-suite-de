...
class Nutrisslim_Product_Tabs_Widget extends Widget_Base {
    ...
    protected function render() {
        ...
        if ( ! empty( $uporaba_content ) ) {
            $tabs['uporaba'] = array(
                'title'    => __( 'Verwendung', 'nutrisslim-suite' ),
        ...
        if ( ! empty( $sestavine_content ) ) {
            $tabs['sestavine'] = array(
                'title'    => __( 'Zutaten', 'nutrisslim-suite' ),
        ...
        if ( ! empty( $hranilne_vrednosti_content ) ) {
            $tabs['hranilne_vrednosti'] = array(
                'title'    => __( 'Nährwerte', 'nutrisslim-suite' ),
        ...
        $tabs['reviews'] = array(
            'title'    => __( 'Bewertungen', 'nutrisslim-suite' ),
...
function ns_render_merged_reviews_panel( int $product_id ) {
    ...
    echo '    <h2 class="woocommerce-Reviews-title">' . esc_html__( 'Kundenbewertungen', 'woocommerce' ) . '</h2>';
...
function nutrisslim_uporaba_tab_content_callback() {
    ...
    if ($warnings) {
        echo '<p><strong>' . __( 'Warnung:', 'nutrisslim-suite' ) . '</strong></p>';
        echo $warnings;
    }
}
