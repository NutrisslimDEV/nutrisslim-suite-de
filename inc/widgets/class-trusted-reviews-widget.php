...
function trusted_get_reviews_block_a() {
    ...
    echo '<h2 class="trusted-review-title">Kundenbewertungen</h2>';
    ...
    echo '<div class="trusted-review-text">Unglaublich</div>';
    ...
    foreach ($comments as $comment) {
        $rating = intval(get_comment_meta($comment->comment_ID, 'rating', true));
        $comment_content = !empty(trim($comment->comment_content)) ? esc_html($comment->comment_content) : esc_html__('Klient zostawił tylko ocenę.', 'woocommerce');
...
function trusted_get_reviews_block_b() {
    ...
    */
    ...
    foreach ($comments as $comment) {
		...
		$comment_content = !empty(trim($comment->comment_content)) ? esc_html($comment->comment_content) : esc_html__('Der Kunde hat nur eine Bewertung hinterlassen.', 'woocommerce');
        ...
        echo '<div class="trusted-review-date">' . sprintf( 
            esc_html__( 'Vor  %s von %s', 'nutrisslim-suite' ), 
            esc_html( $time_diff ), 
            htmlspecialchars( $comment->comment_author, ENT_QUOTES, 'UTF-8' ) 
        ) . '</div>';
        echo '<div class="trusted-revComment">';
        echo '<p class="threerows">' . $comment_content . '</p><p class="rmlink"><a href="#">' . esc_html__( 'Weiterlesen', 'woocommerce' ) . '</a></p>';
        echo '</div>';
        echo '</div>'; // Close trusted-revSlid
        echo '</div>'; // Close swiper-slide
    }
    ...
    if ($best_review) {
        ...
        echo '<p>' . esc_html__('Von', 'woocommerce') . ' ' . esc_html($best_review->comment_author) . '</p>';
...
function trusted_get_reviews_block_c() {
    ...
    echo '<div class="trusted-review-text">Niesamowite</div>';
    $nutri_leaf_url = plugin_dir_url(__FILE__) . 'assets/nutri-leaf.svg';
    echo '<div class="trusted-review-logo"><img src="' . esc_url($nutri_leaf_url) . '" alt="logo" /></div>';
    echo '</div>';
    echo '</div>';
