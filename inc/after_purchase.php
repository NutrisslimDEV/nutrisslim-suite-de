    ...
    <div class="container">
        <div class="grid">
            <div id="orderInfo" class="col-8_sm-12">
                <h4><?php echo __('Bestellübersicht', 'woocommerce'); ?></h4>

                <div class="grid articles">
                    <div class="col-6 label"><?php echo __('Produktname', 'woocommerce'); ?></div>
                    <div class="col-2 text-right label"><?php echo __('Preisangebot', 'woocommerce'); ?></div>
                    <div class="col-2 text-right label"><?php echo __('Menge', 'woocommerce'); ?></div>
                    <div class="col-2 text-right label"><?php echo __('Insgesamt', 'woocommerce'); ?></div>
                    <?php 
                        $totalproducts = 0;
                ...
                <div class="grid summary">
                    ...
                    <div class="col-8 text-right label"><?php echo __('Zwischensumme', 'woocommerce'); ?></div>
                    <div class="col-4 text-right"><?php echo wp_kses_post($order_subtotal); ?></div>
                    <div class="col-8 text-right label">
                        <?php echo __('Versand', 'woocommerce') . '<span class="wo-tax">  (ohne MwSt.)</span>'; ?></div>
                    ...
                    <?php
                            foreach ( $order->get_fees() as $fee ) {
                                ...
                                echo '<div class="col-8 text-right label">' . __(esc_html( $fee_name ), 'nutrisslim-suite') . '<span class="wo-tax"> (ohne MwSt.)</span>' .'</div>';
                                echo '<div class="col-4 text-right">' . $fee_total_incl_tax . '</div>';
                            }
                        ?>

                    <div class="col-8 text-right label znesek"><?php echo __('Insgesamt', 'woocommerce'); ?></div>
                    <div class="col-4 text-right"><?php echo wp_kses_post($order_total); ?></div>
                    <div class="col-8 text-right"><?php echo __('(mit MwSt.)', 'woocommerce'); ?>
                    </div>
            ...
            <div id="customerInfo" class="col-4_sm-12">
                <div class="grid shipping">
                    ...
                    <div style="padding-bottom:0" class="col-12">
                        <h4><?php echo __('Liefer- und Rechnungsadresse', 'nutrisslim-suite'); ?></h4>
                    </div>
            ...
            <div class="col-12 conclusion">
                <p class="text-center">
                    <?php echo __('Vielen Dank für Ihren Einkauf. Wir wünschen Ihnen noch einen schönen Tag!', 'nutrisslim-suite'); ?>
                </p>
                <p class="text-center"><?php echo __('Dein Nature\'s Finest', 'nutrisslim-suite'); ?></p>
            </div>
        </div>
    </div>
</section>
...
function custom_change_order_status_on_cod($order_id) {
    ...
    $interval = $current_time->getTimestamp() - $order_date->getTimestamp(); // Time difference in seconds

    // Check if the payment method is COD, the order status is not completed, and the order is not older than 20 minutes
    if ($order->get_payment_method() == 'cod' && $order->get_status() !== 'completed' && $interval <= 1200) {
        // Change the order status to "on-hold"
        $order->update_status('on-hold', __('Order status changed to On hold due to COD payment method', 'woocommerce'));
    }

    // Check if the payment method is Stripe, the order status is not completed, and the order is not older than 20 minutes
    if ($order->get_payment_method() == 'stripe' && $order->get_status() !== 'completed' && $interval <= 1200) {
        // Change the order status to "on-hold"
        // $order->update_status('on-hold', __('Order status changed to On hold due to Stripe payment method', 'woocommerce'));
    }
}
...
function thankyouContent() {
	...
	if ( is_checkout() && is_wc_endpoint_url('order-received') ) {
		...
		if ( (in_array($method, ['cod', 'stripe'])) && $enabled && ($is_afterpurchase !== 'yes') ) {
			...
			echo '<h3>' . $first_name . ', ' . __('Die Bestellung wurde ausgeführt', 'nutrisslim-suite') . '</h3>';
            echo '<p>' . __('Vielen Dank, dass Sie unsere Produkte gewählt haben.', 'nutrisslim-suite') . '</p>';
            echo '<p>' . __('Bestellnummer', 'woocommerce') . ': <strong>' . $order_id . '</strong></p>';
            echo '<p>' . sprintf(__('Die Bestellbestätigung wird an Ihre E-Mail-Adresse gesendet %1$s.', 'text-domain'), '<strong>' . esc_html($order_email) . '</strong>') . '</p>';
			echo '<p><a id="pregled-narucila" data-oid="' . $order_id . '" class="btn wired-btn elementor-button" href="#">' . __('Bestellübersicht', 'nutrisslim-suite') . ':</a></p>';
            echo '<div class="timer-after-purchase">';
            echo '<h3>' . __('Sonderangebot', 'nutrisslim-suite') . '</h3>';
            echo '<p>' . __('Zusätzlicher Rabatt auf ausgewählte Produkte für eine begrenzte Zeit!!!', 'nutrisslim-suite') . '</p>';
            ...
            if ($has_book_file) {
                ...
                foreach ($order->get_items() as $item_id => $item) {
                    ...
                    if ($book_file) {
                        ...
                        echo '<p class="text-center"><a href="' . esc_url($book_file['url']) . '" target="_blank">' . __('Buch herunterladen', 'nutrisslim-suite') . '</a></p>';
                        echo '</div>'; // col
                        echo '</div>';
                    }
                }
			...
			echo '<div class="container">';
			echo '<h3>' . sprintf(esc_html__('%1$s, Die Bestellung wurde ausgeführt', 'text-domain'), esc_html($first_name)) . '</h3>';
            echo '<p><strong>' . __(' Vielen Dank, dass Sie unsere Produkte gewählt haben.', 'nutrisslim-suite') . '</strong></p>';

            echo '<p>' . __(' Bei Nature\'s Finest engagieren wir uns für die besten natürlichen Lösungen für einen gesunden Lebensstil – und dank Ihnen können wir diese Vision weiterverfolgen!', 'nutrisslim-suite') . '</p>';

            echo '<p>' . __('Wir benachrichtigen Sie, sobald Ihr Paket bereit ist.', 'nutrisslim-suite') . '<br /><br /></p>';
            
            echo '<p>' . __('Bestellnummer', 'woocommerce') . ': <strong>' . $order_id . '</strong></p>';
            
            echo '<p>' . sprintf(__('Die Bestellbestätigung wird an die angegebene E-Mail-Adresse gesendet. <strong>%1$s</strong>.', 'nutrisslim-suite'), esc_html($order_email)) . '</p>';
            if ($order && $order->get_status() === 'pending') {
                echo '<p><a id="show-offer" data-oid="' . $order_id . '" class="btn wired-btn elementor-button" href="#">' . __('Zusätzliches Angebot anzeigen:', 'nutrisslim-suite') . '</a></p>';
            }
...
<?php
            ...
            $ato = sprintf( __( 'Zur Bestellung hinzufügen', 'nutrisslim-suite') );
            $rfo = sprintf( __( 'Aus der Bestellung entfernen', 'nutrisslim-suite') );            
            ...
            if (in_array($method, ['cod'])) {
                ...
                _e('Bestellung abschließen', 'nutrisslim-suite');
                echo '</a>';    
            } else {
            ...
            echo '<p class="text-center"><a id="straighttofinal" data-oid="' . $order_id . '" href="#">' . __( 'Ich möchte dieses Angebot nicht', 'nutrisslim-suite') . '</a></p>';
            echo '</div>';
            ...
            echo '<h3>' . sprintf(__('<span>Ostatnia szansa</span><br /> Rabatt nutzen %1$s%%!', 'nutrisslim-suite'), esc_html($discount_percentage)) . '</h3>';
            ...
            if (in_array($method, ['cod'])) {
                ...
                echo '<p class="text-center"><a class="gotopregled final" data-oid="' . $order_id . '" href="#">' . __( ' Ich lehne dieses Angebot ab or Ich möchte dieses Angebot nicht', 'nutrisslim-suite') . '</a></p>';
            echo '</div>';		
            echo '</div>';
            echo '</div>';		
			...
			echo '<div class="container">';
			echo '<h3>' . sprintf(esc_html__('%1$s, Die Bestellung wurde ausgeführt or Die Bestellung wurde abgeschlossen', 'text-domain'), esc_html($first_name)) . '</h3>';
            echo '<p><strong>' . __('Vielen Dank, dass Sie unsere Produkte gewählt haben.', 'nutrisslim-suite') . '</strong></p>';        
            echo '<p>' . __('Bei Nature\'s Finest engagieren wir uns seit 10 Jahren dafür, die besten natürlichen Lösungen für ein gesundes Leben anzubieten – und dank Ihnen können wir diese Vision weiterverfolgen!', 'nutrisslim-suite') . '</p>';

            echo '<p>' . __('Wir benachrichtigen Sie umgehend, sobald Ihr Paket bereit ist.', 'nutrisslim-suite') . '<br /><br /></p>';
            
            echo '<p>' . __('Bestellnummer', 'woocommerce') . ': <strong>' . $order_id . '</strong></p>';
            
            echo '<p>' . sprintf(__('Die Bestellbestätigung wird an die auf der Website angegebene E-Mail-Adresse gesendet <strong>%1$s</strong>.', 'nutrisslim-suite'), esc_html($order_email)) . '</p>';            if ($order && $order->get_status() === 'pending') {
                echo '<p><a id="show-offer" data-oid="' . $order_id . '" class="btn wired-btn elementor-button" href="#">' . __('Zusätzliches Angebot anzeigen:', 'nutrisslim-suite') . '</a></p>';
            }
...
<div class="grid articles">
    <div class="col-6 label"><?php echo __('Nazwa produktu', 'woocommerce'); ?></div>
    <div class="col-2 text-right label"><?php echo __('Wycena', 'woocommerce'); ?></div>
    <div class="col-2 text-right label"><?php echo __('Ilość', 'woocommerce'); ?></div>
    <div class="col-2 text-right label"><?php echo __('Łącznie', 'woocommerce'); ?></div>
    <?php 
                        $totalproducts = 0;
                        foreach ($order->get_items() as $item_id => $item) :
                            // Skip items with 'nutrisslim_parent_id' meta key
...
<div class="grid summary">
    ...
    <div class="col-8 text-right label"><?php echo __('Suma częściowa', 'woocommerce'); ?></div>
    <div class="col-4 text-right"><?php echo wp_kses_post($order_subtotal); ?></div>
    <div class="col-8 text-right label">
        <?php echo __('Shipping', 'woocommerce') . '<span class="wo-tax"> (ohne MwSt.)</span>'; ?></div>
    <div class="col-4 text-right"><?php echo wp_kses_post($order_shipping); ?>
    </div>
