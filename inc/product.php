...
function add_content_to_footer_on_product_page() {
    if (is_product()) {
        ...
        echo '<h3 id="subscrModalTitle">' . __('Sparen Sie mit einem Abonnement', 'nutrisslim-suite') . '</h3>';  
        echo '<h4>' . __('Vorteile des Abonnements
        ', 'nutrisslim-suite') . '</h4>';
        echo '<ul>';
        echo '<li>' . __(' Wenn Sie sich heute registrieren, wird ein zusätzlicher 5% Rabatt auf den einmaligen Kaufpreis angewendet', 'nutrisslim-suite') . '</li>';
        echo '<li>' . __('Sparen Sie Zeit – erhalten Sie monatliche Lieferungen, ohne eine Bestellung aufgeben zu müssen.', 'nutrisslim-suite') . '</li>';
        //echo '<li>' . __('<strong>Free gift</strong> with every subscribed shipment, starting from the second shipment.', 'nutrisslim-suite') . '</li>';
        echo '<li>' . __('Das Abonnement kann jederzeit in Ihrem Konto im Bereich -> Bestellungen -> Abonnementbereich oder einfach per E-Mail gekündigt werden..', 'nutrisslim-suite') . '</li>';
        // echo '<li>' . __('<strong>Free shipping</strong> from the second shipment onwards.', 'nutrisslim-suite') . '</li>';        
        echo '</ul>';
        echo '<p>' . __('Sie erhalten eine Erinnerungs-E-Mail, bevor das Paket versendet wird.', 'nutrisslim-suite') . '</p>';
        echo '</div>';
        echo '</div>';
    }
}
